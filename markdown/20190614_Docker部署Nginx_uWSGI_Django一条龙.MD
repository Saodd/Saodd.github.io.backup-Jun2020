...
> 端口号一定要是你容器开放的端口号；module后面跟的是你的`WSGI.py`文件的路径，以python风格表示。
...
`Nginx`和`uWSGI`两个东西还是有点坑的，主要是提示很少，到处找日志很费力。

现在他们二者之间还是使用TCPsocket进行通讯，其实还可以用Unixsocket性能更好，
