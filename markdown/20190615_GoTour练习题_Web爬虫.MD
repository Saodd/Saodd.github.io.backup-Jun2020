# Go-Tour 练习题: Web爬虫 ( exercise-web-crawler.go )
...
但是要注意，python的`Queue`是无限长度的，但是Go中的channel是有容量限制的，所以我们设置一个`[]task`来推送任务，
其中`task{depth int, url string}`是我们自定义的数据类型。
