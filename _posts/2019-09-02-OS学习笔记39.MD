...
每个文件都有一个底层文件名，一般是一串数字，这对用户不可见。底层文件名通常称为`索引结点编号inode number`。

文件系统一般不知道文件中到底储存着什么，它只负责帮你保管，并在你请求的时候原样取出。

另一个关键概念是`目录directory`，目录也有索引结点编号，但它内部的数据很特殊，储存的是一系列的`用户文件名-底层文件名`这样的配对。
...
因此我们可以理解为什么删除文件时叫unlink而不是remove了，因为rm工具只是将文件的计数减一。只有当计数为零的时候OS才会释放掉这个索引结点以及相应的数据区块，这时才是真正的删除了。

## 39.15 象征链接（软链接）

硬链接一些限制：你不能给一个目录创建硬链接，防止产生循环；你不能链接到另一个硬盘分区上，因为索引结点号只在分区上唯一。
